...
def setup_scholar_qa(
    reranker_model: str,
    reranker_type: str = "qwen_vllm",
    reranker_llm_model: str = None,
    llm_model: str = None,
    decomposer_model: str = None,
    quote_extraction_model: str = None,
    clustering_model: str = None,
    summary_generation_model: str = None,
...
def main():
    ...
    parser.add_argument(
        "--config-file", type=str, default="config.json", help="Path to configuration file (default: config.json)"
    )
    parser.add_argument(
        "--config-name", type=str, default="default", help="Configuration name to use from the config file (default: default)"
    )
    parser.add_argument(
        "--output-prefix",
        type=str,
        help="Optional prefix for the output filename (e.g., 'experiment1' -> 'experiment1_scholar_query_results_...')",
    )
    parser.add_argument(
        "--reranker",
        type=str,
    ...
    parser.add_argument("--fallback-model", type=str, help=f"The fallback LLM model to use. If not specified, uses GPT_41.")
    parser.add_argument(
        "--table-column-model",
        type=str,
